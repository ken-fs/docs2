...
/**
 * shadcn 的 Tabs，改成印刷厂的「铅字滑块」：
 * 一块墨色实心 Indicator 在标签间滑动，滑动用 ease-snap（带轻微过冲）。
...
const tabsListVariants = cva("group/tabs-list relative z-1 flex items-stretch", {
  variants: {
    variant: {
      /** 墨块在框内滑动，整组带硬描边 —— 用在结果区的 source / preview */
      slab: "border border-rule-firm bg-paper/70",
      /** 只有底部一条墨线跟着走 —— 用在正文里不抢戏的地方 */
      rule: "gap-4 border-b border-rule",
    },
  },
  ...
  return (
    ...
    >
      ...
      <TabsPrimitive.Indicator
        ...
        className={cn(
          "absolute left-0 -z-1 w-(--active-tab-width) translate-x-(--active-tab-left) bg-ink",
          "transition-[translate,width] duration-200 ease-snap",
  ...
  return (
    <TabsPrimitive.Tab
      ...
      className={cn(
        ...
        "group-data-[variant=rule]/tabs-list:data-active:text-ink",
