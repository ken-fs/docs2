...
export type Dictionary = {
  ...
  converter: {
    ...
    /**
     * 本站有好几个地方都收，而且哪个都可能对 —— 后面跟的是一串链接。
     *
     * 为什么不能复用 wrongType：那句是「这一页不收，那一页收」，一个确定的
     * 答案。而 .txt 有三页收，三个不同的引擎（Markdown / 纯文本 / CSV），
     * 光看扩展名分不出里面装的是哪一种。「This one does:」后面跟三个链接
     * 读起来是错的（this one 指哪个？），而且它没说清为什么有三个 ——
     * 得让用户知道这是要他自己认，不是让他随便挑一个。
     */
