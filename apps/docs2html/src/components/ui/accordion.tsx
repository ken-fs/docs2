...
/**
 * shadcn 的 Accordion，重写成蓝图那一套。
 * 改掉的东西：圆角、chevron 换成一个旋转的小方块（像图纸上的节点标记）、
 * ring 焦点环换成实线描边、不再引 lucide。
 * 高度过渡用 Base UI 给的 --accordion-panel-height，缓动是 ease-halt（急停，不是 ease-in-out）。
 */
...
function AccordionItem({ className, ...props }: AccordionPrimitive.Item.Props) {
  return (
    <AccordionPrimitive.Item
      ...
      className={cn("border-b border-grid last:border-b-0", className)}
      {...props}
    />
  );
}
...
function AccordionContent({
  ...
  return (
    <AccordionPrimitive.Panel
      ...
      className="h-(--accordion-panel-height) overflow-hidden transition-[height] duration-200 ease-halt data-ending-style:h-0 data-starting-style:h-0"
      {...props}
    >
      <div
        className={cn(
          "pb-4 pl-[18px] text-[14px] leading-relaxed text-graphite-soft",
          className,
        )}
      >
        {children}
