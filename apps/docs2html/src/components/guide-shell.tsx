    ...
    <>
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-5 sm:px-8">
        ...
        <article className="max-w-[44rem] pt-7 pb-16 lg:pt-9">
          ...
          <div className="mt-5 space-y-2 text-[15px] leading-relaxed text-graphite-soft">
            {page.lede.map((line) => (
              <p key={line}>{line}</p>
            ))}
          </div>
          ...
          <Link
            href={toolHref}
            className="mt-6 inline-flex items-center gap-1.5 font-mono text-[11px] tracking-[0.1em] text-prussian uppercase underline decoration-dotted underline-offset-4 transition-colors hover:text-prussian-deep"
          >
            {g.cta}
          ...
          {page.sections.map((section) => (
            <section
              key={section.heading}
              className="mt-10 border-t border-grid pt-6"
            >
              <h2 className="font-display text-xl leading-tight text-graphite">
                {section.heading}
              </h2>
              {section.body && (
                <div className="mt-3 space-y-3 text-[14px] leading-relaxed text-graphite-soft">
              ...
              {section.steps && (
                <ol className="mt-3.5 space-y-2.5 text-[14px] leading-relaxed text-graphite-soft">
                  {section.steps.map((step, i) => (
                    <li key={step} className="flex gap-3">
                      <span className="mt-[1px] shrink-0 font-mono text-[11px] text-prussian tabular-nums">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {[
                    ...
                    <figure key={label} className="min-w-0">
                      <figcaption className="font-mono text-[10px] tracking-[0.18em] text-graphite-faint uppercase">
                        {label}
                      </figcaption>
                      {/* 横向滚动而不是折行：这里面是代码，折行会把一个标签
                          切成两半，读的人分不清哪个换行是原文里的。 */}
                      <pre className="mt-1.5 overflow-x-auto border border-grid-firm bg-sheet-deep px-3 py-2.5 font-mono text-[11.5px] leading-relaxed text-graphite">
                        <code>{code}</code>
                      </pre>
                    </figure>
                  ))}
          ...
          <section className="mt-12 border-t border-grid-firm pt-7">
            <p className="text-[14px] leading-relaxed text-graphite-soft">
              {page.outro}
            </p>
            <Link
              href={toolHref}
              className="plate-lift mt-4 inline-flex items-center gap-2 border border-prussian bg-sheet px-4 py-2 font-mono text-[11px] tracking-[0.1em] text-prussian uppercase hover:bg-prussian hover:text-sheet"
            >
          ...
          <nav className="mt-12 border-t border-grid pt-6">
