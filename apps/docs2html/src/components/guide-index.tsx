 ...
 *
 * 每张卡片有两个可点的东西（整张卡去文章，底下那行去工具页）。<a> 不能套
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-5 sm:px-8">
        ...
        <div className="pt-7 pb-16 lg:pt-9">
          ...
          <ul className="mt-10 max-w-[52rem] border-t border-grid-firm">
            {GUIDE_KEYS.map((k) => {
              const guide = dict.guides[k];
              return (
                <li
                  key={k}
                  className="group border-b border-grid py-5 transition-colors duration-150 hover:border-grid-firm"
                >
                  <h2 className="font-display text-[19px] leading-snug text-graphite">
                    <Link
                      href={guidePath(locale, k)}
                      className="transition-colors duration-150 group-hover:text-prussian"
                    >
                      {guide.h1}
                    </Link>
                  </h2>
                  <p className="mt-2 max-w-[44rem] text-[13.5px] leading-relaxed text-graphite-soft">
                    {guide.description}
                  </p>
                  {/* 「配哪个工具」那行用中性色，只有工具页名是蓝的 ——
                      一行里主色只标真正能点的那几个字。 */}
                  <p className="mt-2.5 font-mono text-[11px] text-graphite-faint">
                    {c.guide.pairedWith}{" "}
                    <Link
                      href={pathOf(locale, guide.tool)}
                      className="inline-flex items-baseline gap-1 text-prussian transition-colors hover:text-prussian-deep"
                    >
